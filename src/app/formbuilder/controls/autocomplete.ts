...
export default class controlAutocomplete extends control {
  ...
  build() {
    ...
    const keyboardNav = (e) => {
      ...
      let direction : any = keyCodeMap.get(e.keyCode);
      if(!direction) {
        direction = () => false;
      }
    ...
    const field = [
      super.markup('input', null, fauxAttrs),
      super.markup('input', null, hiddenAttrs)
    ];
    ...
    const options = values.map(optionData => {
      ...
      return super.markup('li', label, config);
    });

    field.push(super.markup('ul', options,
